...
const AssignDriver = () => {
    ...
    useEffect(() => {
        // Fetch all drivers (You might need a GetDrivers endpoint in your API)
        DataService.get("/AdminDriver/GetDrivers").then(res => setDrivers(res.data));
    }, []);

    const handleConfirm = async () => {
        try {
            await DataService.post(`/AdminBus/AssignBusToDriver?bus_id=${busId}&driver_id=${selectedDriver}`);
    ...
    return (
        <div style={styles.container}>
            <div style={styles.card}>
                <h2 style={{color: '#7e57c2'}}>Assigning to: Bus {busNumber}</h2>
                <label>Select Available Driver</label>
                <select style={styles.input} onChange={(e) => setSelectedDriver(e.target.value)}>
                    <option>Select a driver...</option>
                    {drivers.map(d => <option key={d.driver_id} value={d.driver_id}>{d.name}</option>)}
                </select>
                <button style={styles.btnConfirm} onClick={handleConfirm}>Confirm Assignment</button>
            </div>
        </div>
    );
};
...
const styles = {
    container: { display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: '#1a233a' },
    card: { backgroundColor: '#263238', padding: '40px', borderRadius: '20px', width: '350px', color: 'white' },
    input: { width: '100%', padding: '12px', margin: '15px 0', borderRadius: '8px' },
    btnConfirm: { width: '100%', padding: '12px', backgroundColor: '#7e57c2', color: 'white', border: 'none', borderRadius: '8px' }
};
