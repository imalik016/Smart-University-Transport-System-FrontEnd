...
import { useNavigate } from "react-router-dom";
import DataService from "../../services/axios"; // Adjust path as needed
import Sidebar from "./Sidebar"; // Updated name from bottomNavbar

export default function BusAttendance() {
  const navigate = useNavigate();
  
  // State to hold data retrieved from DB
  const [student, setStudent] = useState({ name: "Loading...", regNo: "..." });
  ...
  const fetchStudentData = async () => {
    try {
      setLoading(true);
      // Assuming you store the user's ID in localStorage upon login
      const userId = localStorage.getItem("user_id"); 
      
      // Integration with your Backend Controller (e.g., StudentController/GetProfile)
      const res = await DataService.get(`/Student/GetProfile`, {
        params: { id: userId }
      });
      ...
      if (res.data) {
        setStudent({
          name: res.data.name,
          regNo: res.data.registration_no || res.data.regNo
        });
      }
    } catch (error) {
      console.error("Error retrieving student info from DB:", error);
      // Fallback or error handling
      setStudent({ name: "Error Loading", regNo: "N/A" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={styles.container}>
      ...
      {/* Main Layout Wrapper - Needed to push content away from Sidebar */}
